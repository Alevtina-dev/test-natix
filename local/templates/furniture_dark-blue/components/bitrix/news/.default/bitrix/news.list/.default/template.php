...
?>
    ...
    <!-- Расширенное меню фильтрации -->
        <div class="list_year">
            <a href="<?=$APPLICATION->GetCurPageParam('', array('year','month'));?>"<?if (!isset($_GET['year'])):?> class="active"<?endif;?>><?=GetMessage("ALL_TIME")?></a>
            <? foreach ($arResult["YEARS"] as $year): ?>
                <a href="<?=$APPLICATION->GetCurPageParam('year='.$year["NAME"], array('year','month'));?>"<?if ($year["ACTIVE"]):?> class="active"<?endif;?>><?=$year["NAME"]?></a>
            <?endforeach;?>
        </div>
        <? if (!empty($arResult["MONTH"])):?>
            <div class="list_month">
                <? foreach ($arResult["MONTH"] as $month): ?>
                    <a href="<?=$APPLICATION->GetCurPageParam('month='.$month["VALUE"], array('month'));?>"<?if ($month["ACTIVE"]):?> class="active"<?endif;?>><?=$month["NAME"]?></a>
                <?endforeach;?>
            </div>
        <?endif;?>
    <!-- end Расширенное меню фильтрации -->
